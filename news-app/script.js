
const APIkey="YOUR_API_KEY";
const url="https://newsapi.org/v2/everything?q=";
const body=document.querySelector("body");
const container= document.querySelector("#card-container");
const newCard=document.getElementById("cards-maker");
const darkBtn=document.querySelector("#dark")
let i=0;
darkBtn.addEventListener("click",function(){
   if(i==0){
      body.style.backgroundColor="black";
      darkBtn.innerHTML="Disbale";
      i=1;
   }
   else if(i==1){
      body.style.backgroundColor="white";
      darkBtn.innerHTML="Enable";
      i=0;
   }


});
window.addEventListener("load",fetchnews('India'));

    async function fetchnews(query)
    {
       const Response= await fetch(`${url}${query}&apiKey=${APIkey}`);
       const data=  await Response.json();
       
       bindData(data.articles);
       if(!data.articles)
       {
            const nodisp=document.createElementbyTag("h1")
           nodisp.style.fontSize="300px";
           container.appendChild(nodisp)

       }
    }


     function bindData(articles){
   

    container.innerHTML=""
     
     articles.forEach(function(article)
     {
      //   if(!article.urlToImage) return;
        
        const cardClone= newCard.content.cloneNode(true);
       filldata(cardClone,article);

        container.appendChild(cardClone);
        
        
     })
    
     }
function filldata(cardClone ,article)

{
   const newImg=cardClone.querySelector("#newImg");
   const newtitle=cardClone.querySelector("#title");
   const newPara=cardClone.querySelector("#paragraph");
   const newSource=cardClone.querySelector("#source");
   newImg.src=article.urlToImage;
   newtitle.innerHTML=article.title;
   newPara.innerHTML=article.description;
   if(!article.urlToImage){
      newImg.src="data:image/jpeg;base64,"
   }

   const date=new Date(article.publishedAt).toLocaleString("en-US",{
      timeZone: "Asia/Jakarta"
   });
newSource.innerHTML=`${article.source.name}.${date}`;
cardClone.firstElementChild.addEventListener("click",function()
{
   window.open(article.url,"_blank")
})
}
const input=document.querySelector("#input");
const submit=document.querySelector("#submit");
submit.addEventListener("click",function()
{
  const query=input.value;
  if(!query) return;
  fetchnews(query);
});
curNav=null
function NavClick(id)
{
fetchnews(id);
const navItem =document.getElementById(id);
curNav?.classList.remove('active');
curNav=navItem;
curNav.classList.add('active');
}
